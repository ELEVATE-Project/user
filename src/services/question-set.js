...
const questionsSetQueries = require('../database/queries/question-set')
...
module.exports = class questionsSetHelper {
	...
	static async create(bodyData, decodedToken) {
		try {
			...
			let questionSet = await questionsSetQueries.findOneQuestionSet(questionSetData)
			if (questionSet) {
				return responses.failureResponse({
			...
			questionSet = await questionsSetQueries.createQuestionSet(questionSetData)
	...
	static async update(questionSetId, bodyData, decodedToken) {
		try {
			...
			const questionSet = await questionsSetQueries.updateOneQuestionSet(filter, questionSetData)
	...
	static async read(questionsSetId, questionSetCode) {
		try {
			...
			const questionSet = await questionsSetQueries.findOneQuestionSet(filter)
			if (!questionSet) {
				return responses.failureResponse({
					message: 'QUESTION_NOT_FOUND',
					statusCode: httpStatusCode.bad_request,
