...
module.exports = class AccountHelper {
  ...
  static async login(bodyData) {
    ...
    try {
      let user = await usersData.findOne(
        { "email.address": bodyData.email.toLowerCase() },
        projection
      );
      if (!user) {
        return common.failureResponse({
  ...
  static async generateOtp(bodyData) {
    try {
      ...
      const userData = await redisCommunication.getKey(
  ...
  static async resetPassword(bodyData) {
    ...
    try {
      ...
      const redisData = await redisCommunication.getKey(
        bodyData.email.toLowerCase()
      );
      if (!redisData || redisData.otp != bodyData.otp) {
        return common.failureResponse({
      ...
      const deleteData = await redisCommunication.deleteKey(
        bodyData.email.toLowerCase()
      );
  ...
  static async bulkCreateMentors(mentors, tokenInformation) {
    return new Promise(async (resolve, reject) => {
      try {
        const systemUser = await systemUserData.findUsersByEmail(
          tokenInformation.email
        );
