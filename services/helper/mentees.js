...
module.exports = class MenteesHelper {
  ...
  static async reports(userId, filterType) {
    ...
    try {
      ...
      totalSessionEnrolled = await sessionAttendees.countSessionAttendees(
        filterStartDate,
        filterEndDate,
        userId
      );
      ...
      totalsessionsAttended =
        await sessionAttendees.countSessionAttendeesThroughStartDate(
          filterStartDate,
          filterEndDate,
          userId
        );
  ...
  static async getAllSessions(page, limit, search, userId) {
    ...
    const sessions = await sessionData.findAllSessions(
      page,
      limit,
      search,
      filters
      ...
      await Promise.all(
        sessions[0].data.map(async (session) => {
          const attendee = attendees.find(
            (attendee) =>
              attendee.sessionId.toString() === session._id.toString()
          );
  ...
  static async getMySessions(page, limit, search, userId) {
    ...
    const sessions = await sessionAttendees.findAllUpcomingMenteesSession(
      page,
      limit,
      search,
      filters
