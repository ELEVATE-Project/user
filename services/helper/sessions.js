...
module.exports = class SessionsHelper {
    ...
    static async create(bodyData,loggedInUserId) {
        ...
        try {
           let data = await sessionData.createSession(bodyData);
            return common.successResponse({ statusCode: httpStatusCode.created, message: apiResponses.SESSION_CREATED_SUCCESSFULLY, result:data });
        } catch (error) {
            throw error;
        }
    }
    ...
    static async update(sessionId, bodyData,loggedInUserId) {
        ...
        try {
            const result = await sessionData.updateOneSession({ _id: ObjectId(sessionId) }, bodyData);
            if (result === 'SESSION_ALREADY_EXISTS') {
                return common.failureResponse({ message: apiResponses.SESSION_ALREADY_EXISTS, statusCode: httpStatusCode.bad_request, responseCode: 'CLIENT_ERROR' });
            ...
            return common.successResponse({ statusCode: httpStatusCode.accepted, message: apiResponses.SESSION_UPDATED_SUCCESSFULLY  });
        } catch (error) {
            throw error;
        }
    }
