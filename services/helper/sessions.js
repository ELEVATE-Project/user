...
const apiBaseUrl =
  process.env.USER_SERIVCE_HOST + process.env.USER_SERIVCE_BASE_URL;
...
module.exports = class SessionsHelper {
  ...
  static async create(bodyData, loggedInUserId) {
    ...
    try {
      ...
      if (bodyData.startDate) {
        bodyData["startDateUtc"] = moment
          .unix(bodyData.startDate)
          .utc()
          .format(common.UTC_DATE_TIME_FORMAT);
      }
      if (bodyData.endDate) {
        bodyData["endDateUtc"] = moment
          .unix(bodyData.endDate)
          .utc()
          .format(common.UTC_DATE_TIME_FORMAT);
      }

      let elapsedMinutes = moment(bodyData.endDateUtc).diff(
  ...
  static async update(sessionId, bodyData, userId, method) {
    ...
    try {
      ...
      const sessionDetail = await sessionData.findSessionById(
        ObjectId(sessionId)
      );
      ...
      if (method == common.DELETE_METHOD || isSessionReschedule) {
        const sessionAttendees =
          await sessionAttendesData.findAllSessionAttendees({
        ...
        const attendeesAccounts =
          await sessionAttendeesHelper.getAllAccountsDetail(
            sessionAttendeesIds
          );
        ...
        sessionAttendees.map((attendee) => {
          for (
            let index = 0;
            index < attendeesAccounts.result.length;
            index++
          ) {
  ...
  static async details(id, userId = "") {
    try {
      ...
      const sessionDetails = await sessionData.findOneSession(
        filter,
        projection
      );
      ...
      if (userId) {
        let sessionAttendee = await sessionAttendesData.findOneSessionAttendee(
          sessionDetails._id,
          userId
        );
  ...
  static async list(loggedInUserId, page, limit, search, status) {
    try {
      ...
      const sessionDetails = await sessionData.findAllSessions(
        page,
        limit,
        search,
        filters
      ...
      if (
        sessionDetails[0] &&
        sessionDetails[0].data.length == 0 &&
        search !== ""
      ) {
        return common.failureResponse({
          message: apiResponses.SESSION_NOT_FOUND,
  ...
  static async enroll(sessionId, userTokenData, timeZone) {
    ...
    try {
      ...
      const sessionAttendeeExist =
        await sessionAttendesData.findOneSessionAttendee(sessionId, userId);
  ...
  static async unEnroll(sessionId, userTokenData) {
    ...
    try {
      ...
      const response = await sessionAttendesData.unEnrollFromSession(
        sessionId,
        userId
      );
  ...
  static upcomingPublishedSessions(page, limit, search) {
    return new Promise(async (resolve, reject) => {
      try {
        const publishedSessions = await sessionData.searchAndPagination(
          page,
          limit,
          search
        );
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (session.link) {
          ...
          let elapsedMinutes = moment(session.startDateUtc).diff(
            currentDate,
            "minutes"
          );
          ...
          const moderatorMeetingLink =
            await bigBlueButton.joinMeetingAsModerator(
              session._id,
              mentorDetails.name,
              session.mentorPassword
            );
